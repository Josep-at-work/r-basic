...
En la carpeta scripts del tema4 se encuentra el markdown de ejemplos de joanby.
...
par(mfrow = c(1,2)) #Formato de multiples graficos, 1 fila con 2 graficos.
...
xlim, ylim igualar a un vector(c) con el limite inferior y superior de cada eje.
Tanto para yaxp como xaxp pide tres argumentos, (inicio, final, numero de divisiones)
...
- `points(x,y)`: añade un punto de coordenadas $(x, y)$ a un gráfico ya existente.**Pueden ser un vector de puntos**
...
- `curve(curva)`: permite añadir la gráfica de una curva a un gráfico existente
    ...
    - A diferencia de de la anterior, `curve()` te permite añadir una función cualquiera sin tener que hacer un plot adicional.
...
- `legend(posición, legend = ...)`: para añadir una leyenda
    ...
    - `col`, `lwd`, `lty`y otras propiedades de las linias se pueden añadir en forma de vector referenciando cada una a un valor de `legend` o lo que sería lo mismo a las distintas partes que queremos diferenciar del gráfico.
...
polygon(c(20,30,40),c(10,-10,10), col = "gold", density = 50, angle = 90, lty = 4, 
        lwd = 5)
```
